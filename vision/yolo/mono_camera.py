...
class ObjectDetector:
    ...
    def __init__(self, detection_config: dict = None):
        ...
        # 初始化YOLO模型
        self.model = YOLO('best.pt')  # 使用best.pt模型
...
class VisionGuidanceSystem:
    ...
    def process_frame(self) -> Tuple[Optional[np.ndarray], Optional[DroneCommand]]:
        ...
        if detections:
            ...
            # 绘制检测结果 - 显示所有检测到的动物
            self._draw_all_detections(frame, detections, best_detection, command)
    ...
    def _draw_all_detections(self, frame: np.ndarray, detections: List[Dict], 
                           best_detection: Dict, command: Optional[DroneCommand]):
        """在画面上绘制所有检测结果"""
        # 绘制所有检测框
        for i, detection in enumerate(detections):
            ...
            # 判断是否为当前目标
            is_target = detection == best_detection
            color = (0, 255, 0) if is_target else (255, 255, 0)  # 绿色为目标，黄色为其他
        ...
        # 显示控制信息（针对目标动物）
        if command and best_detection:
            info_text = f"Target: {best_detection.get('class_name', 'unknown')}"
            cv2.putText(frame, info_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            
            cmd_text = f"Cmd: F:{command.velocity_forward:.2f} R:{command.velocity_right:.2f} D:{command.velocity_down:.2f}"
            cv2.putText(frame, cmd_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        ...
        if best_detection:
            ...
            cv2.putText(frame, error_text, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            
            # 显示偏移补偿状态
            if abs(error_x) < 0.1 and abs(error_y) < 0.1:
                cv2.putText(frame, "Offset Compensation: ON", (10, 120), 
        ...
        # 显示检测统计
        stats_text = f"Detected: {len(detections)} animals"
    ...
    def _draw_detection(self, frame: np.ndarray, detection: Dict, command: Optional[DroneCommand]):
        """在画面上绘制检测结果（保留原方法兼容性）"""
        self._draw_all_detections(frame, [detection], detection, command)
