...
class SerialPort:
    ...
    def _process_buffer(self):
        ...
        while True:
            ...
            # 查找包尾
            packet_end = self.rx_buffer.find(LORA_PACKET_FOOTER, packet_start)
            if packet_end == -1:
                # 包尾还没接收到，等待更多数据
                break
            
            # 提取完整数据包
            packet = self.rx_buffer[packet_start:packet_end + len(LORA_PACKET_FOOTER)]
            self.receivetime = 1
            # 解析数据内容
            ...
            self.receivetime += 1
            # 从缓冲区移除已处理的数据
            self.rx_buffer = self.rx_buffer[packet_end + len(LORA_PACKET_FOOTER):]
